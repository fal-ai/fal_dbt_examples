...
def find_ideal_min_samples(X: np.array, range_min_samples: list):
    ...
    for window in range(size_0):
        for i in range(len(range_min_samples)):
            ...
            print(f'for window {window} and min_samples {i} calc complete')
...
model_df = ref(context.current_model.name).sort_values(by='x')
...
column_date = model_df['x'].to_numpy(dtype=np.int64).reshape(-1,1)

print('data read and sliced')

window_size = 9000#100<z

range_min_samples = [2,3,4,5]
min_samples = find_ideal_min_samples(column_y, range_min_samples)

print('found min_samples')
